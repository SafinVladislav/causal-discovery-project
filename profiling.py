...
def setup_for_profiling():
    true_model = create_true_model()  # From graphs.py, adjust as needed
    obs_data = silent_simulate(true_model, OBS_SAMPLES)  # From intervention.py
    essential_graph = to_undirected_with_v_structures(nx.DiGraph(true_model))  # From run_simulation.py
    return essential_graph, obs_data, true_model
